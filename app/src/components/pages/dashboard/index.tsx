...
import { Area, AreaChart, XAxis, Bar, BarChart, Line, LineChart } from "recharts"
import { TrendingUp, TrendingDown, Users, Activity, DollarSign, Loader2, Check, MoreVertical, Plus } from "lucide-react"
...
function ColorSwatch({ name, variable, colorClass }: { name: string; variable: string; colorClass: string }) {
    return (
        <div className="flex flex-col gap-2">
            <div
                className={`h-16 w-full rounded-lg border ${colorClass}`}
            />
                ...
                <div className="flex items-center justify-between">
                    <div className="space-y-1">
                        ...
                        <p className="text-muted-foreground">
                            Showcasing all design tokens from globals.css
                        </p>
                    </div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-6">
                                {shadows.map((shadow) => (
                                    <div key={shadow.name} className="space-y-2">
                                        <div
                                            className={`h-16 w-full bg-card rounded-lg border ${shadow.class}`}
                                        />
                                        <div className="space-y-0.5">
